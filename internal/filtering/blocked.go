...
// FilterUnknownIDs removes unknown service IDs from s.IDs.  s must not be nil.
func (s *BlockedServices) FilterUnknownIDs(_ context.Context, _ *slog.Logger) {
	known := make(map[string]struct{}, len(serviceIDs))
	for _, id := range serviceIDs {
		known[id] = struct{}{}
	}

	s.IDs = slices.DeleteFunc(s.IDs, func(id string) (del bool) {
		_, ok := known[id]
