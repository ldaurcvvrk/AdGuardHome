...
var nextFilterID = rules.ListID(time.Now().Unix())
...
func (filter *FilterYAML) Path(dataDir string) string {
	return filepath.Join(dataDir, filterDir, strconv.FormatUint(uint64(filter.ID), 10)+".txt")
}
...
func deduplicateFilters(filters []FilterYAML) (deduplicated []FilterYAML) {
	...
	for _, filter := range filters {
		if _, exists := urls[filter.URL]; !exists {
			urls[filter.URL] = struct{}{}
...
func assignUniqueFilterID() rules.ListID {
	value := nextFilterID
	nextFilterID++
	return value
}
...
func (d *DNSFilter) enableFiltersLocked(async bool) {
	...
	err := d.setFilters(context.TODO(), filters, allowFilters, async)
	if err != nil {
