...
// Registrar is the interface for registering HTTP handlers.
type Registrar interface {
	// Register registers a handler for the URL.
	Register(method, url string, handler http.HandlerFunc)
}
...
// ErrorAndLog writes formatted message to w, logs it using l, and sets the
// response status code.  l must not be nil.
func ErrorAndLog(
	ctx context.Context,
	l *slog.Logger,
	r *http.Request,
	...
	text := fmt.Sprintf(format, args...)
	l.ErrorContext(ctx, "request failed", "method", r.Method, "url", r.URL, "error", text)
	http.Error(w, text, code)
}
