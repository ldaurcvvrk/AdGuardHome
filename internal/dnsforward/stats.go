...
func (s *Server) processQueryLogsAndStats(
	...
	// Record enhanced DNS analysis data.
	s.processAnalysis(ctx, l, dctx, s.analysis)
...
func (s *Server) updateStats(dctx *dnsContext, clientIP string, processingTime time.Duration) {
	...
	if len(upstreamStats) > 0 {
		upstream = upstreamStats[0].Address
		upstreamTime = upstreamStats[0].QueryDuration
	}
