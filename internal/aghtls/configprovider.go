// Package aghtls provides TLS configuration management.
package aghtls
...
// TLSConfigProvider provides TLS configuration to consumers.
// Implementations must be safe for concurrent use.
//
// TODO(m.kazantsev):  Merge with the Manager interface.
type TLSConfigProvider interface {
// TLSConfig returns a clone of the current TLS configuration.
// conf provides its certificates via GetConfigForClient method.
...
// ExtendedTLSConfig returns a clone of the current extended TLS configuration.
ExtendedTLSConfig() (conf *ExtendedTLSConfig)

// SetExtendedTLSConfig updates the current extended TLS configuration.
// It returns true if the configuration was changed.
// servePlainDNS is used to determine whether to serve DNS over plain UDP/TCP.
