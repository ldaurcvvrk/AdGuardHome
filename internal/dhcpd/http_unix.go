...
func (s *server) handleDHCPStatus(w http.ResponseWriter, r *http.Request) {
	...
	aghhttp.WriteJSONResponseOK(r.Context(), s.logger, w, r, status)
}

func (s *server) enableDHCP(ifaceName string) (code int, err error) {
	var hasStaticIP bool
	ctx := context.Background()
	hasStaticIP, err = aghnet.IfaceHasStaticIP(ctx, nil, ifaceName)
	if err != nil {
	...
	if !hasStaticIP {
		err = aghnet.IfaceSetStaticIP(ctx, s.logger, nil, ifaceName)
		if err != nil {
...
func (s *server) handleDHCPInterfaces(w http.ResponseWriter, r *http.Request) {
	...
	for _, iface := range ifaces {
		...
		jsonIface, iErr := newNetInterfaceJSON(r.Context(), s.logger, iface)
		if iErr != nil {
...
func newNetInterfaceJSON(ctx context.Context, l *slog.Logger, iface net.Interface) (out *netInterfaceJSON, err error) {
	...
	out.GatewayIP = aghnet.GatewayIP(ctx, l, nil, iface.Name)
...
func (s *server) handleDHCPFindActiveServer(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	if isStaticIP, serr := aghnet.IfaceHasStaticIP(ctx, nil, ifaceName); serr != nil {
		...
		result.V4.StaticIP.IP = aghnet.GetSubnet(ctx, s.logger, ifaceName).String()
	}

	setOtherDHCPResult(ifaceName, result)

	aghhttp.WriteJSONResponseOK(r.Context(), s.logger, w, r, result)
}
