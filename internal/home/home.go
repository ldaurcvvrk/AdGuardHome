...
type homeContext struct {
	...
	// dnsAnalysis is the enhanced DNS analysis module.  It must be passed to
	// [dnsforward.DNSCreateParams.Analysis] when the DNS server is created.
	dnsAnalysis *dnsanalysis.Analysis
...
func run(
	...
	// Initialize the enhanced DNS analysis module.  It is enabled by default
	// and persists its data in the data directory.
	globalContext.dnsAnalysis, err = dnsanalysis.New(&dnsanalysis.Config{
		Logger:  baseLogger.With(slogutil.KeyPrefix, "dns_analysis"),
		HTTPReg: httpReg,
		DataDir: dataDirPath,
		Enabled: true,
	})
	...
	globalContext.dnsAnalysis.Start()
	...
	if globalContext.dnsAnalysis != nil {
		err = globalContext.dnsAnalysis.Close()
		if err != nil {
			baseLogger.ErrorContext(ctx, "closing dns analysis", slogutil.KeyError, err)
		}
	}
}
