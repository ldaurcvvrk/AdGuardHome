...
// IsCommentOrEmpty returns true if s is empty or a comment line.
func IsCommentOrEmpty(s string) bool {
	s = strings.TrimSpace(s)
	return s == "" || strings.HasPrefix(s, "#")
}

// UpstreamHTTPVersions converts a boolean to the corresponding HTTP versions
// slice for upstream configuration.
func UpstreamHTTPVersions(useHTTP3 bool) []upstream.HTTPVersion {
	if useHTTP3 {
		return []upstream.HTTPVersion{upstream.HTTPVersion11, upstream.HTTPVersion2, upstream.HTTPVersion3}
	}
	return []upstream.HTTPVersion{upstream.HTTPVersion11, upstream.HTTPVersion2}
}
