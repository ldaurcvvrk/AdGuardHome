...
func (c *searchCriterion) ctFilteringStatusCase(
	...
	case filteringStatusFiltered:
		return isFiltered || reasonIn(reason,
...
func (c *searchCriterion) isFilteredWithReason(reason filtering.Reason) (matched bool) {
	...
	case filteringStatusBlocked:
		return reasonIn(reason, filtering.FilteredBlockList, filtering.FilteredBlockedService)
...
// reasonIn returns true if reason is equal to one of rs.
func reasonIn(reason filtering.Reason, rs ...filtering.Reason) (ok bool) {
	for _, r := range rs {
		if reason == r {
			return true
		}
