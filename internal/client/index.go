...
func newIndex() (ci *index) {
	sortedMap := aghalg.NewSortedMap[netip.Prefix, UID](subnetCompare)
	return &index{
		subnetToUID:   &sortedMap,
		nameToUID:     map[string]UID{},
		clientIDToUID: map[ClientID]UID{},
		ipToUID:       map[netip.Addr]UID{},
		macToUID:      map[macKey]UID{},
...
func (ci *index) add(c *Persistent) {
	...
	for _, id := range c.ClientIDs {
		ci.clientIDToUID[ClientID(id)] = c.UID
	}
...
func (ci *index) clashes(c *Persistent) (err error) {
	...
	for _, id := range c.ClientIDs {
		existing, ok := ci.clientIDToUID[ClientID(id)]
...
// clashesIP returns a previous client with the same IP address as c or nil.
// c must be non-nil.
...
// clashesSubnet returns a previous client with the same subnet as c or nil.
// c must be non-nil.
func (ci *index) clashesSubnet(c *Persistent) (p *Persistent, s netip.Prefix) {
	for _, s = range c.Subnets {
		...
		ci.subnetToUID.Range(func(pref netip.Prefix, uid UID) (cont bool) {
			if s == pref {
				existing = uid
				ok = true
				return false
...
// clashesMAC returns a previous client with the same MAC as c or nil.
// c must be non-nil.
...
func (ci *index) findByIP(ip netip.Addr) (c *Persistent, found bool) {
	...
	ci.subnetToUID.Range(func(pref netip.Prefix, uid UID) (cont bool) {
		// Remove zone before checking because prefixes don't have zones.
		// See TODO on [queryLogEntry.IP].
		if pref.Contains(ipWithoutZone) {
			c = ci.uidToClient[uid]
			found = true
			return false
		}
...
// findByIPWithoutZone finds a persistent client by IP address without zone.
// It strips the zone from the stored IP addresses before comparing.
// This is useful because query log entries don't have zones.
// See TODO on [queryLogEntry.IP].
...
// findByMACWithoutZone finds a persistent client by IP address without zone.
// It strips the zone from the stored IP addresses before comparing.
// This is useful because query log entries don't have zones.
// See TODO on [queryLogEntry.IP].
func (ci *index) findByMACWithoutZone(ip netip.Addr) (c *Persistent) {
	if ip == (netip.Addr{}) {
		return nil
	}
...
// remove removes information about persistent client from the index.  c must
// be non-nil.
func (ci *index) remove(c *Persistent) {
	delete(ci.nameToUID, c.Name)

	for _, id := range c.ClientIDs {
		delete(ci.clientIDToUID, ClientID(id))
	}
